...
public class PermissionRequester {
    ...
    public static final int NOT_SUPPORT_VERSION = 2; /** 요청 AndroidOS의 버젼이 마쉬멜로우 이상 버젼이 아닐 경우 */

    public static final int ALREADY_GRANTED = -1;   /** 요청 권한을 이미 가지고 있을 경우 */

    public static final int REQUEST_PERMISSION = 0; /** 권한을 System에게 요청한 경우 -> Activity의 onRequestPermissionsResult() 로 결과 리턴됨. */
    ...
    // 해당 앱이 특정 권한을 가지고 있는지 검사
        ...
        if (permissionCheck == PackageManager.PERMISSION_DENIED) {
            ...
            //권한을 취할 떄 사용자의 허락을 받아야하는가?
                // 1. 전에 사용자가 권한 요청을 거부한 적이 있다 => True
                // 2. 전에 사용자가 권한 요청을 거부한 적이 없다 => False

                AlertDialog.Builder dialog = new AlertDialog.Builder(context);
                //권한 취득해야 하는 이유를 Dialog 등을 통해서 알림
                ...
                dialog.setTitle(builder.getTitle()) .setMessage(builder.getMessage()).setPositiveButton(builder.getPositiveButtonName(), new DialogInterface.OnClickListener() {
                    ...
                    @Override public void onClick(DialogInterface dialog, int which) {
                        denyAction.onClick(context); } }).create().show(); return REQUEST_PERMISSION;
            } else {
                ...
                context.requestPermissions(new String[]{permission}, requestCode);
                return REQUEST_PERMISSION; } } else {
